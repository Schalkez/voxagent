...
```bash
node "C:/Users/PC/Works/Personal/test/huggingface/voxagent/.claude/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "C:/Users/PC/Works/Personal/test/huggingface/voxagent/.claude/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="C:/Users/PC/Works/Personal/test/huggingface/voxagent/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in C:/Users/PC/Works/Personal/test/huggingface/voxagent/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf C:/Users/PC/Works/Personal/test/huggingface/voxagent/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
